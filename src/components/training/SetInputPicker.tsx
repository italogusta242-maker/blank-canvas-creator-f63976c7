    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-[360px] p-0 bg-background border-border overflow-hidden">
        ...
        <div className="flex gap-6 px-4 items-center">
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            ...
            <input
              ...
              className="w-24 h-12 text-center text-2xl font-bold bg-secondary/80 border border-border/50 rounded-xl text-foreground outline-none focus:border-primary transition-colors tabular-nums"
            />
          </div>
        </div>
        ...
        <div className="p-4 pt-2">
          <Button
            onClick={handleSave}
            className="w-full py-4 rounded-2xl font-bold text-lg text-white border-0 shadow-lg shadow-primary/20 transition-all hover:scale-[1.02] active:scale-[0.98]"
            style={{ background: "linear-gradient(135deg, hsl(336, 100%, 65%), hsl(336, 100%, 45%))" }}
          >
            SALVAR
          </Button>
        </div>
