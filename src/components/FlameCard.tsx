...
import { Flame, Shield, CircleAlert, Trophy } from "lucide-react";
import type { FlameState } from "@/hooks/useFlameState";
import { getNextReward, calculateFlameIntensity } from "@/lib/flameMotor";
...
const FlameCard = ({ state, streak, adherence, className = "" }: FlameCardProps) => {
  ...
  // Combine basic adherence with the logic from flameMotor to generate a compelling progress visual
  const intensity = calculateFlameIntensity(streak);
  ...
  return (
    <motion.div
      ...
      className={`rounded-xl border p-6 flex flex-col items-center ${className} relative overflow-hidden`}
      style={{
    ...
    >
      ...
      <p className="text-[11px] mt-1" style={{ color: config.subtitleColor }}>
        {config.subtitle(adherence)} (Intensidade: {intensity}%)
      </p>

      {/* Reward Milestone Badge */}
      {nextReward && state !== "extinta" && (
        <div className="mt-5 w-full bg-secondary/30 rounded-lg p-3 border border-secondary/50 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Trophy size={14} className="text-amber-500" />
            <span className="text-xs font-semibold text-foreground/80">{nextReward.title}</span>
          </div>
          <span className="text-[10px] uppercase font-bold text-muted-foreground">
            FALTAM {nextReward.days - streak} DIAS
          </span>
        </div>
      )}
    </motion.div>
