...
const MonthlyAssessment = () => {
  ...
  useEffect(() => {
    ...
    const prefillForm = async () => {
      ...
      if (lastAssessment) {
        const la = lastAssessment as any;
        setForm(prev => ({
          ...prev,
          modalidade: la.modalidade || prev.modalidade,
          dias_disponiveis: la.dias_disponiveis || prev.dias_disponiveis,
          frequencia_compromisso: la.frequencia_compromisso || prev.frequencia_compromisso,
          tempo_disponivel: la.tempo_disponivel || prev.tempo_disponivel,
          maquinas_indisponiveis: la.maquinas_indisponiveis || prev.maquinas_indisponiveis,
          objetivo_atual: la.objetivo_atual || prev.objetivo_atual,
          competicao_fisiculturismo: la.competicao_fisiculturismo || prev.competicao_fisiculturismo,
          restricao_alimentar: la.restricao_alimentar || prev.restricao_alimentar,
          alimentos_proibidos: la.alimentos_proibidos || prev.alimentos_proibidos,
          refeicoes_horarios: la.refeicoes_horarios || prev.refeicoes_horarios,
          horario_treino: la.horario_treino || prev.horario_treino,
          horario_treino_outro: la.horario_treino_outro || prev.horario_treino_outro,
          prioridades_fisicas: la.prioridades_fisicas || prev.prioridades_fisicas,
        }));
        setPrefilled(true);
        return;
      }
    ...
    <div>
      ...
      <div className="flex gap-1 mt-1 flex-wrap">
        {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
          <button key={n} type="button" onClick={() => u(field, String(n) as MonthlyFormData[keyof MonthlyFormData])}
