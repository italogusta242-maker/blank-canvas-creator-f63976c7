...
import { Plus, Edit3, CreditCard, QrCode, Repeat, Package, Loader2, Trash2 } from "lucide-react";
...
const AdminPlanosPagamento = () => {
  ...
  const billingIcon = (type: string) => {
    if (type === "recurring") return <Repeat size={14} className="text-primary" />;
    return <QrCode size={14} className="text-primary/70" />;
  };
              ...
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                      {billingIcon(plan.billing_type || "recurring")}
                      <span className="text-xs text-muted-foreground">
                        {billingLabel(plan.billing_type || "recurring")}
                      </span>
                      <span className="text-xs text-muted-foreground">·</span>
                      <span className="text-xs text-muted-foreground">
                        {plan.billing_type === "recurring" ? "Cartão" : paymentMethodLabel(plan.payment_method || "PIX")}
                      </span>
                      {plan.billing_type === "one_time" && plan.payment_method === "CREDIT_CARD" && (plan.max_installments || 1) > 1 && (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        toggleMutation.mutate({ id: plan.id, active: checked })
                      }
                    />
                  </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
                ...
                <Select value={form.payment_method} onValueChange={(v) => setForm({ ...form, payment_method: v, ...(v === "PIX" ? { max_installments: "1" } : {}) })}>
                  ...
                  <SelectContent>
                    <SelectItem value="PIX">
                      <span className="flex items-center gap-2"><QrCode size={14} /> PIX</span>
                    </SelectItem>
                    <SelectItem value="CREDIT_CARD">
                      <span className="flex items-center gap-2"><CreditCard size={14} /> Cartão de Crédito</span>
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
            ...
            {/* Max installments - only for one_time + credit card */}
...
export default AdminPlanosPagamento;
