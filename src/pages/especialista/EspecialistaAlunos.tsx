...
import { useSpecialistStudents, useMySpecialty, type StudentWithDetails } from "@/hooks/useSpecialistStudents";
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("training_plans")
        .select("id, title, active, updated_at, groups, objetivo_mesociclo")
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              Atualizado {new Date(data.updated_at).toLocaleDateString("pt-BR")} · {groups.length} grupo(s)
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
...
const StudentResumoContent = ({ aluno, specialty }: {
  ...
  <div className="space-y-5">
    ...
    <div className="border-t border-border/50" />
    <p className="text-sm text-muted-foreground text-center py-3">Informações de perfil do aluno</p>
        ...
        {hasTabs ? (
          <Tabs defaultValue="resumo" className="mt-2">
            ...
            <TabsContent value="resumo" className="mt-4">
              <StudentResumoContent aluno={aluno} specialty={specialty} />
            </TabsContent>
            {showTreino && (
...
const EspecialistaAlunos = () => {
  ...
  const { data: dietPlans } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("diet_plans")
        .select("id, user_id, title, goal_description, meals, updated_at, active")
  ...
  const dietPlanSet = new Set((dietPlans ?? []).map((d: any) => d.user_id));
  ...
  const { data: dietAdherenceMap } = useQuery({
    ...
    queryFn: async () => {
      ...
      (data ?? []).forEach((h) => {
        ...
        prev.done += Array.isArray(h.completed_meals) ? (h.completed_meals as any[]).length : 0;
  ...
  const { data: trainingPlans } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("training_plans")
        .select("id, user_id, title, groups, updated_at, active, objetivo_mesociclo")
        .in("user_id", studentIds)
        .eq("active", true);
      if (error) throw error;
      return data ?? [];
  ...
  const trainingPlanSet = new Set((trainingPlans ?? []).map((t: any) => t.user_id));
  ...
  (trainingPlans ?? []).forEach((p: any) => {
    if (!trainingPlanInfoMap.has(p.user_id)) {
      ...
      trainingPlanInfoMap.set(p.user_id, {
        ...
        totalSessions: groupsArr.length,
        updated_at: p.updated_at,
        user_id: p.user_id,
      });
    }
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-muted-foreground">
          {filterParam === "alerta"
            ...
            : "Visualize o perfil completo e acompanhe o progresso"}    </p>
      </div>
