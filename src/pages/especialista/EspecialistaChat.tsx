    ...
    const load = async () => {
      ...
      const [profilesRes, convsRes, allPartsRes, lastMsgsRes] = await Promise.all([
        ...
        convIds.length > 0
          ...
          : Promise.resolve({ data: [] }),
        convIds.length > 0
          ? supabase.from("chat_messages").select("conversation_id, content, created_at, sender_id, media_type").in("conversation_id", convIds).order("created_at", { ascending: false })
          : Promise.resolve({ data: [] }),
      ]);
