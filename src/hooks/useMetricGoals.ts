...
export function useMetricGoals() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      (data ?? []).forEach((r: any) => { map[r.metric_key] = Number(r.target_value); });
      return map;
    },
  });
}
  ...
  return useMutation({
    mutationFn: async ({ metric_key, goal_value }: MetricGoal) => {
      ...
      const { error } = await supabase
        .from("metric_goals")
        .upsert({ metric_key, metric_name: metric_key, target_value: goal_value, user_id: user.id } as any, { onConflict: "metric_key" } as any);
      if (error) throw error;
    },
